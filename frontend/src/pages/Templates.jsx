  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2">Diyet Şablonları</h1>
          <p className="text-gray-400">Diyet oluştururken kullanacağınız öğün düzenlerini yönetin.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
             ...
             <div className="col-span-full text-center py-12 bg-[#1a1a2e] border border-white/5 rounded-2xl">
                <Layout className="w-12 h-12 text-gray-600 mx-auto mb-4" />
                <h3 className="text-lg font-medium text-white mb-2">Henüz Şablon Yok</h3>
                <p className="text-gray-400">Yeni bir şablon oluşturarak başlayın.</p>
             </div>
        ) : (
            templates.map((template) => (
                <div 
                    ...
                    className="group bg-[#1a1a2e] border border-white/5 hover:border-purple-500/30 rounded-2xl p-6 transition-all duration-300 hover:shadow-xl hover:shadow-purple-900/10 cursor-pointer relative overflow-hidden"
                >
                    <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                         <button 
                            onClick={(e) => openEditModal(template.id, e)}
                            className="p-2 bg-white/5 hover:bg-blue-500/20 hover:text-blue-400 rounded-lg text-gray-400 transition-colors"
                         >
                            <Edit size={16} />
                         </button>
                         <button 
                            onClick={(e) => handleDelete(template.id, e)}
                            className="p-2 bg-white/5 hover:bg-red-500/20 hover:text-red-400 rounded-lg text-gray-400 transition-colors"
                         >
                            <Trash2 size={16} />
                         </button>
                    </div>
                    ...
                    <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <span className="text-xs font-medium text-gray-500 bg-white/5 px-2 py-1 rounded mt-1 inline-block">
                                Otomatik Düzen
                            </span>
                        </div>
                    </div>
