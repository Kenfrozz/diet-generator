...
from PyQt6.QtGui import QFont, QIcon, QMouseEvent, QPixmap, QPainter, QPainterPath
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # Ayarlar ve Kullanıcılar hariç tüm butonları ekle
        for i, (icon_name, text) in enumerate(self.menu_items[:-1]):  # Son öğe hariç (Ayarlar)
            btn = QPushButton()
        ...
        # Alt grup butonları (Sadece Ayarlar)
        for i in range(len(self.menu_items) - 1, len(self.menu_items)):
            icon_name, text = self.menu_items[i]
            btn = QPushButton()
    ...
    def logout_clicked(self):
        ...
        if login_dialog.exec() == LoginDialog.DialogCode.Accepted:
            ...
            # Ayarlara kullanıcı bilgisini gönder
            self.settings.set_user(self.current_user)
