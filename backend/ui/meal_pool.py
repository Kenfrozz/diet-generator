...
class MealPoolWidget(QWidget):
    ...
    def __init__(self, db, pool_type=None):
        ...
        self.db = db
        # Filtre durumları
        self.current_pool_type = pool_type or "normal"  # Varsayılan normal havuz
        self.current_meal_type = None      # Varsayılan tüm öğünler
        
        # Havuz renkleri
        self.pool_colors = {
            "normal": "#3498db",  # Mavi
            "hastalik": "#e74c3c" # Kırmızı
        }
    ...
    def setup_ui(self):
        ...
        # Arama ikonu
        search_icon = get_colored_icon("search.svg", "#9999aa")
        ...
        self.filter_btn.setIcon(get_colored_icon("filter.svg", "#9999aa"))
        ...
        self.filter_btn.setMenu(self._create_filter_menu())
    ...
    def _create_filter_menu(self):
        ...
        # Havuz Başlığı
        title = QAction("HAVUZ SEÇİMİ", self)
        title.setEnabled(False)
        menu.addAction(title)
        
        # Normal Havuz
        normal = QAction("Normal Havuz", self)
        normal.setCheckable(True)
        normal.setChecked(self.current_pool_type == "normal")
        normal.triggered.connect(lambda: self._set_filter("pool", "normal"))
        menu.addAction(normal)
        
        # Hastalık Havuzu
        hastalik = QAction("Hastalık Havuzu", self)
        hastalik.setCheckable(True)
        hastalik.setChecked(self.current_pool_type == "hastalik")
        hastalik.triggered.connect(lambda: self._set_filter("pool", "hastalik"))
        menu.addAction(hastalik)
        
        menu.addSeparator()
        
        # Öğün Başlığı
        meal_title = QAction("ÖĞÜN SEÇİMİ", self)
        meal_title.setEnabled(False)
        menu.addAction(meal_title)
        
        # Tümü
        all_meals = QAction("Tüm Öğünler", self)
        all_meals.setCheckable(True)
        all_meals.setChecked(self.current_meal_type is None)
        all_meals.triggered.connect(lambda: self._set_filter("meal", None))
        menu.addAction(all_meals)
        
        # Diğer öğünler
        for key, name in MEAL_TYPES.items():
            meal_action = QAction(name, self)
            meal_action.setCheckable(True)
            meal_action.setChecked(self.current_meal_type == key)
            meal_action.triggered.connect(lambda checked, k=key: self._set_filter("meal", k))
            menu.addAction(meal_action)
            
        return menu

    def _set_filter(self, filter_type, value):
        """Filtreyi uygula ve menüyü güncelle."""
        if filter_type == "pool":
            self.current_pool_type = value
        elif filter_type == "meal":
            self.current_meal_type = value
            
        # Menüyü yeniden oluştur (seçimleri güncellemek için)
        self.filter_btn.setMenu(self._create_filter_menu())
    ...
    def refresh_recipes(self):
        ...
        meal_type = self.current_meal_type
        ...
        for row, recipe in enumerate(page_recipes):
            ...
            # Tarif Adı - Büyük harf
            name = recipe["name"].replace('i', 'İ').upper()
            self.table.setItem(row, 1, QTableWidgetItem(name))
            
            # Havuz ismi
            pool_raw = recipe.get("pool_type", "")
            pool_name = pool_raw.replace('i', 'İ').upper()
            pool_item = QTableWidgetItem(pool_name)
            
            if pool_raw and hasattr(self, 'pool_colors'):
                pool_color = self.pool_colors.get(pool_raw, "#6b7280")
                pool_item.setForeground(QColor(pool_color))
            self.table.setItem(row, 2, pool_item)
            
            # Öğün türü
            meal_type = MEAL_TYPES.get(recipe["meal_type"], recipe["meal_type"])
            self.table.setItem(row, 3, QTableWidgetItem(meal_type.replace('i', 'İ').upper()))
